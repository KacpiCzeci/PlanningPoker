...
export default function GamePage(){
    ...
    const Vote = async () => {
        ...
        }).then(res => CalculateResult()).catch(error => console.log("Error: Vote fetch http://localhost:3000/Endgame", error));
    }
    /**
    ...
    useEffect(()=>{
        ...
        .then(res =>{
            ...
            /**Players&Score */
            setResults(res.slice(1,2)[0].players.map((i:any)=>{return i.score}));
            console.log("Results: "+results);

            /**Game State */
            setGameState(res.slice(-1)[0].gameState)
            console.log(gameState);
        }).catch(error => console.log("Error: GetResults fetch http://localhost:3000/GetResult", error));
        }
